...
def init_lookup_table(z, REWRITE_TBLS = False):
    ...
    if os.path.exists(FOLDERPATH):
        ...
        if (os.path.isfile(FPATH) and not REWRITE_TBLS):
            ...
            hmf = MassFunction(Mmin = 9, Mmax = 18, dlog10m = 0.1, lnk_min = -11.1, lnk_max = 14.6,
                               dlnk=0.000025, z=z, hmf_model = "Behroozi", sigma_8 = 0.8159,
...
def PS_1_2h(k, z, crit_dens_rescaled, M_h_array, HMF_array, N_G, NCEN, NSAT, bias):
...
def omega_inner_integral(theta, z, comoving_distance_z, crit_dens_rescaled, M_h_array, HMF_array,
                         N_G, NCEN, NSAT, k_array, dlogk, bias, hmf_PS):
    PS_1_2 = np.array([PS_1_2h(k, z, crit_dens_rescaled, M_h_array, HMF_array, N_G, NCEN, NSAT,
                                bias)*k/(2*np.pi)*special.j0(k*1/206265*comoving_distance_z)
                                for k in k_array])
    ...
    R_T1 = np.sum(k_array * PS_1_2.T[0] * corr, axis = -1) * dlogk
    R_T2 = np.sum(k_array * hmf_PS * PS_1_2.T[1] * corr, axis = -1) * dlogk
    return R_T1, R_T2
...
def omega_z_component_single(args):
    ...
    theta, M_min, sigma_logM, M_sat, alpha, NCEN, NSAT, REWRITE_TBLS = _args_
    shmem = SharedMemory(name=f'{mem_id}', create=False)
    shres = np.ndarray(shape, buffer=shmem.buf, dtype=np.float64)

    M_h_array, HMF_array, nu_array, k_array, hmf_PS = init_lookup_table(z, REWRITE_TBLS)
    dlogk = np.log(k_array[1]/k_array[0])
    bias = Tinker10(nu=nu_array).bias()
    N_G  = n_g(M_min, sigma_logM, M_sat, alpha, M_h_array, HMF_array)
    comoving_distance_z = cosmo.comoving_distance(z).value
    crit_dens_rescaled = (4/3*np.pi*cosmo.critical_density(z).value*200*2e40)
    oz1, oz2 = omega_inner_integral(theta, z, comoving_distance_z, crit_dens_rescaled, M_h_array,
                                    HMF_array, N_G, NCEN, NSAT, k_array, dlogk, bias, hmf_PS)
    shres[job_id, 0, :] = oz1
    shres[job_id, 1, :] = oz2
    return

def omega_z_component_parallel(z_array, theta_array, M_min, sigma_logM, M_sat, alpha,
                               NCEN, NSAT, REWRITE_TBLS, cores=None):
    if cores is None:
    ...
    _args_ = theta_array, M_min, sigma_logM, M_sat, alpha, NCEN, NSAT, REWRITE_TBLS
...
def omega(theta, M_min, sigma_logM, M_sat, alpha, N_z_nrm, z_array,
    ...
    M_h_array, __, ___, ____, _____ = init_lookup_table(0, REWRITE_TBLS)
    ...
    itg = omega_z_component_parallel(z_array, theta, M_min, sigma_logM, M_sat, alpha,
                                     NCEN, NSAT, REWRITE_TBLS, cores)
